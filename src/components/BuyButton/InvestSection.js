...
const InvestSection = () => {
  ...
  const updateBalanceAction = isDefault
    ? updateOrderUSDVRP
    : updateOrderUSDVDAO;
  ...
  const disabledState =
    isPending || !userAgreed || !isStarted || orderedBalance < 1
      ? " btn--disabled"
      : "";
  ...
  const currentStage =
    currentAllowance() >= orderedBalance && orderedBalance > 0
      ? config.buyStages.buy
      : config.buyStages.approve;

  function CurrentAllowanceSetup(value) {
    const newAllowance = {
      VRPUSDT:
        currencyIsDefault && isDefault ? value : State.approvedValues.VRPUSDT,
      VAOUSDT:
        currencyIsDefault && !isDefault ? value : State.approvedValues.VAOUSDT,
      VRPBUSD:
        !currencyIsDefault && isDefault ? value : State.approvedValues.VRPBUSD,
      VAOBUSD:
        !currencyIsDefault && !isDefault ? value : State.approvedValues.VAOBUSD,
    };
  ...
  const Invest = async () => {
    pendingState(true);
    try {
      const usdBalance = await RequestWalletBalance(
        State.account,
        usdTokenList.get(currency)
      );
      dispatch(
        currencyIsDefault
          ? updateWalletUSDT(usdBalance)
          : updateApprovedBUSD(usdBalance)
      );
      if (!usdBalance || usdBalance < orderedBalance) {
        dispatch(selectWindow(config.windowNames.nobalance));
        pendingState(false);
        return;
      }
    } catch (e) {
    ...
    try {
      const res = await Buy(currentContract(), State.account, orderedBalance);
      pendingState(false);
    } catch (e) {
      pendingState(false);
      return;
    ...
    const requestingContracts = isDefault
      ? [config.saleContractAddrVRPUSDT, config.saleContractAddrVRPBUSD]
      : [config.saleContractAddrVDAOUSDT, config.saleContractAddrVDAOBUSD];

    const lastLocked = isDefault ? State.lockedVRP : State.lockedVDAO;
    setTimeout(() => {
      RequestLockedFunds(requestingContracts, State.account).then(
        (res, rej) => {
          if (res) {
            if (res > lastLocked) {
  ...
  const MaxUSD = async () => {
    const MaxTokens = await RequestMax(
      usdTokenList.get(currency),
      State.account
    );
  ...
  const RequestWithdraw = async () => {
    const isWithdrawn = await WithdrawTokens(
      currentContract(),
      withdrawalAmount * config.decimal,
      State.account
    );
    if (isWithdrawn) {
    ...
    switch (currentStage) {
      case config.buyStages.install:
        btn.push(
          <div
            key="stage0"
            className={`confirm--button ${btnAddClass} install--stage${disabledState}`}
          >
      ...
      case config.buyStages.connect:
        btn.push(
          <div
            key="stage1"
            className={`confirm--button ${btnAddClass} connect--stage${disabledState}`}
          >
    ...
    <div className="invest--container" onLoad={IsSaleStart}>
      <div className="invest--count--section section--from">
        <div className="invest--num">
          <div className="invest--num--title">from</div>
          <div className="invest--num--count">
            <input
              type="number"
              className="order--input"
              value={orderedBalance}
              onChange={UpdateOrderUSD}
            />
          </div>
        </div>
        <div className="invest--select">
          <div className="currency--selector">
            <div className="max--value--btn" onClick={MaxUSD}>
              MAX
            </div>
            <div
              className="selected--token"
              onClick={SwitchCurrency.bind(this, "selectCurrency")}
            >
              <img
                src={
                  currency === config.defaultCurrency
                    ? "images/icons/USDT.png"
                    : "images/icons/BUSD.png"
                }
                width="30"
                height="30"
              />
      ...
      <div className="invest--count--section section--to">
        <div className="invest--num">
          <div className="invest--num--title">to</div>
        ...
        <div className="invest--select">
          <div className="currency--selector">
            ...
            <div
              className="selected--token"
              onClick={SwitchCurrency.bind(this, "selectToken")}
            >
              <img
                src={
                  State.token === config.defaultToken
                    ? "images/icons/VRP.png"
                    : "images/icons/VAO.png"
                }
                width="30"
                height="30"
              />
      ...
      <div className="invest--description">
        <p>
          You need to agree with terms and conditions and then to approve tokens
          in amount not smaller than you will buy. When you'll buy it you will
          get it on locked balance. Then it's will unlocked through westing
          period.
          <a href={config.howToUrl} target="_blank">
            How to buy?
          </a>
        </p>
      </div>
      <div className="buy--confirm--section">
        <div className="agree--dot">
          ...
          <p>
            <a href={config.policyUrl} target="_blank">
              I accept all terms and conditions
            </a>
          </p>
        </div>
        <div className="buy--button--section final--button">{btn}</div>
      </div>
